# Create the main prediction model class
class T20MatchPredictor:
    ...
    def get_head_to_head_advantage(self, team1, team2):
        ...
        matchup = (team1, team2) if (team1, team2) in self.h2h_data else (team2, team1)
        if matchup not in self.h2h_data:
            return 0  # No historical data
            
        h2h = self.h2h_data[matchup]
        if matchup == (team1, team2):
            advantage = (h2h[f'{team1.lower()}_wins'] - h2h[f'{team2.lower()}_wins']) / h2h['matches'] * 100
        else:
            advantage = (h2h[f'{team1.lower()}_wins'] - h2h[f'{team2.lower()}_wins']) / h2h['matches'] * 100
            advantage = -advantage  # Reverse since teams are swapped
...
predictor = T20MatchPredictor(teams_df, head_to_head_data, pitch_conditions, 
                             weather_conditions, injury_impact, recent_form_data)

print("T20I Cricket Match Prediction Model initialized successfully!")
print(f"Model includes {len(teams_df)} teams with comprehensive statistics")
print(f"Head-to-head data for {len(head_to_head_data)} matchups")
print(f"Pitch conditions: {list(pitch_conditions.keys())}")
print(f"Weather conditions: {list(weather_conditions.keys())}")